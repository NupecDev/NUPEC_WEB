 ...
 * "Felino Hepatic" (id: product-felino-felino-hepatic).
 *
 * El contenido de Problema/Solución es el mismo que el de Hepatic canino
 * (ver scripts/patch-problem-solution-hepatic.ts) — según la ficha técnica
 * (NUP_VADEMECUM_CLIN_ESP_Problema-Solucion.pdf) esta tabla se comparte
 * entre especies para la línea Hepatic, solo cambia por producto.
 *
 * Usa `setIfEmpty` para no pisar el campo si ya se hubiera llenado
 * manualmente en Sanity Studio.
 *
 * Uso:
 *   npx tsx scripts/patch-problem-solution-felino-hepatic.ts
 */
...
const PRODUCT_ID = "product-felino-felino-hepatic";
...
async function run() {
  console.log(`\n🐾  NUPEC – Patch: Problema y Solución — Felino Hepatic`);
