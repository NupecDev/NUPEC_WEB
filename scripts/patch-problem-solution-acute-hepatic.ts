 ...
 * "Acute Hepatic" (canino, id: product-canino-acute-hepatic).
 *
 * El contenido de Problema/Solución es el mismo que el de Hepatic (ver
 * scripts/patch-problem-solution-hepatic.ts) — se comparte entre productos
 * de la misma línea hepática, según indicación directa.
 *
 * Usa `setIfEmpty` para no pisar el campo si ya se hubiera llenado
 * manualmente en Sanity Studio.
 *
 * Uso:
 *   npx tsx scripts/patch-problem-solution-acute-hepatic.ts
 */
...
const PRODUCT_ID = "product-canino-acute-hepatic";
...
async function run() {
  console.log(`\n🐾  NUPEC – Patch: Problema y Solución — Acute Hepatic (canino)`);
